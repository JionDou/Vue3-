...
const columns: ColumnProps[] = [
  ...
  },
  {
    ...
    width: 100,
    render: ({ row }) => {
      return row.authStatus === 0 ? <el-tag type="warning">未认证</el-tag> : <el-tag>已认证</el-tag>
    }
  },
  {
    prop: 'createTime',
...
const openDialog = async (title: string, row?: INormalMange.IResNormal) => {
  // 检查是否有操作权限
  const isAuth = title === '新增' ? hasAuth('btn.UserNormal.add') : hasAuth('btn.UserNormal.update')
